    ...
    <div class="basis-1/2">
        <ul class="flex">
            ...
            <li class="px-4  mt-10 text-[17px] flex">  
              <form action="{{url('/selectHall')}}" method="GET" style="margin-top: -40px;" class="flex text-white px-2  rounded-md">
                  
                <select name="hall" id="hall" style="background: transparent; width:150px" class="font-mono" >
...
<p class="text-white font-mono text-[18px]  mb-3"><span class="font-semibold text-[20px]">Price : </span>{{$movie->price}} <span>Show Time :{{$movie->show_time}}</p>
 ...
 <p class="text-white mb-2">
  @if ($isEmailVerified)
  <a href="{{url('/ticket', $movie->id)}}" class="btn bg-red-500 px-5 py-3 rounded-md  text-white text-center font-mono text-[19px]">Buy Ticket</a>
@else
    <p class="text-white mb-2 bg-red-500 px-1 py-3 rounded-md">Email is not verified</p>
@endif
 </p>
